...
In this unit we move from first-order differential equaitons to __second-order differential equations__, that is, differential equations involving a second (but no higher) derivative. Examples of such equations are
...
To take a simple case, consider a particle of mass $m$ that moves in one simension along the $x$-axis. At any given time $t$, the particle's position is $x(t)$, and its velocity and acceleration are given by the derivatives $dx/dt$ and $d^2x/dt$. There are no general laws for the positons or velocity of the particle, but there is a very important law for its acceleration: $\mbox{Newton's second law}$ tells us that
...
This equation may be decomposed into two first-order equationsas follows:
<p align='center'>
